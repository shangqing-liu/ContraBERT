...
from parser.dfg import (remove_comments_and_docstrings,
                       tree_to_token_index,
                       index_to_code_token)
...
for lang in dfg_function:
    LANGUAGE = Language('./parser/languages/my-languages.so', lang)
    parser = Parser()
...
def convert_examples_to_features(item):
    ...
    code_tokens = [tokenizer.sep_token] + code_tokens + [tokenizer.sep_token]
    ...
    length = len([tokenizer.sep_token])
    dfg_to_code = [(x[0] + length, x[1] + length) for x in dfg_to_code]
    # nl
    nl = ' '.join(js['docstring_tokens'])
    nl_tokens = tokenizer.tokenize(nl)[:args.nl_length - 2]
    nl_tokens = [tokenizer.sep_token] + nl_tokens + [tokenizer.sep_token]
...
def train(args, model, tokenizer, pool):
    ...
    for idx in range(args.num_train_epochs):
        ...
        if results['eval_mrr'] > best_mrr:
            ...
            no_stop_count += 1
            if no_stop_count > args.early_stop:
                logger.info("No improvements for %d epoches, current best mrr %f" % (args.early_stop, best_mrr))
                break
...
def main():
    ...
    parser.add_argument('--early_stop', type=int, default=5, help="early stop training process")
